...
function ragFromRisk(risk: Risk): RagStatus {
  if (risk.status === "closed" || risk.status === "mitigated") return "green";
  if (risk.probability === "high" && risk.impact === "high") return "red";
  if (risk.probability === "high" || risk.impact === "high") return "amber";
  return "green";
}

function ragFromIssue(issue: Issue): RagStatus {
  if (issue.status === "closed" || issue.status === "resolved") return "green";
  if (issue.severity === "critical") return "red";
  if (issue.severity === "high") return "amber";
  return "green";
}

function ragFromAction(action: ActionItem, today = todayIsoDate()): RagStatus {
  if (action.status === "completed" || action.status === "cancelled") return "green";
  const daysUntilDue = daysBetween(today, action.dueDate);
  if (daysUntilDue !== null && daysUntilDue < 0) return "red";
  if (daysUntilDue !== null && daysUntilDue <= 7) return "amber";
  return "green";
}

function ragFromDependency(dependency: Dependency, today = todayIsoDate()): RagStatus {
  if (dependency.status === "resolved") return "green";
  const daysUntilDue = daysBetween(today, dependency.dueDate);
  if (dependency.status === "blocked" || (daysUntilDue !== null && daysUntilDue < 0)) return "red";
  if (daysUntilDue !== null && daysUntilDue <= 7) return "amber";
  return "green";
}

function isWithinDays(days: number | null, start: number, end: number) {
  return days !== null && days >= start && days <= end;
}
...
export function ProjectFileWorkspace() {
  ...
  function navigateTo(view: NavigationItem, targetId?: string) {
    setActiveView(view);
    if (targetId) {
  ...
  function navigateToRaid(tab: "Risks" | "Assumptions" | "Issues", targetId?: string) {
    setActiveRaidTab(tab);
    navigateTo("RAID", targetId);
  }
  ...
  function renderDashboard(file: PmgovFile) {
    ...
    const attentionMilestones: DashboardMilestone[] = file.milestones
      .map((milestone) => ({ milestone, ...getMilestoneContext(file, milestone), reasons: getMilestoneAttentionReasons(milestone, today), daysUntilPlanned: daysBetween(today, milestone.plannedDate) }))
    ...
    const topRisks = file.risks.filter((risk) => risk.status !== "closed" && risk.status !== "mitigated").sort((a, b) => (ragFromRisk(b) === "red" ? 2 : ragFromRisk(b) === "amber" ? 1 : 0) - (ragFromRisk(a) === "red" ? 2 : ragFromRisk(a) === "amber" ? 1 : 0)).slice(0, 5);
    const criticalIssues = file.issues.filter((issue) => issue.severity === "critical" && issue.status !== "resolved" && issue.status !== "closed");
    const overdueActions = openActions.filter(({ daysUntilDue }) => daysUntilDue !== null && daysUntilDue < 0);
    const recentDecisions = [...file.decisions].sort((a, b) => b.decisionDate.localeCompare(a.decisionDate)).slice(0, 5);
    const thisWeekMilestones = upcomingMilestones.filter((item) => isWithinDays(item.daysUntilPlanned, 0, 7));
    const next30Milestones = upcomingMilestones.filter((item) => isWithinDays(item.daysUntilPlanned, 0, 30));
    const thisWeekActions = openActions.filter((item) => isWithinDays(item.daysUntilDue, 0, 7));
    const next30Actions = openActions.filter((item) => isWithinDays(item.daysUntilDue, 0, 30));
    const kpis = [
      { label: "Portfolio Health", value: ragLabel(calculatedProjectHealth.status), status: calculatedProjectHealth.status, detail: `${calculatedProjectHealth.reasons.length} health signal(s)` },
      { label: "Projects Requiring Attention", value: attentionMilestones.length + criticalIssues.length + blockedDependencies.length, status: attentionMilestones.some((item) => item.milestone.status === "red") || criticalIssues.length || blockedDependencies.length ? "red" : attentionMilestones.length ? "amber" : "green", detail: "Milestones, issues, blockers" },
      { label: "Critical Issues", value: criticalIssues.length, status: criticalIssues.length ? "red" : "green", detail: "Open or investigating" },
      { label: "Top Risks", value: topRisks.length, status: topRisks.some((risk) => ragFromRisk(risk) === "red") ? "red" : topRisks.some((risk) => ragFromRisk(risk) === "amber") ? "amber" : "green", detail: "Highest active exposure" },
      { label: "Blocked Dependencies", value: blockedDependencies.length, status: blockedDependencies.length ? "red" : "green", detail: "Status is blocked" },
      { label: "Upcoming Milestones", value: next30Milestones.length, status: next30Milestones.some((item) => item.milestone.status === "red") ? "red" : next30Milestones.some((item) => item.milestone.status === "amber") ? "amber" : "green", detail: "Next 30 days" },
      { label: "Overdue Actions", value: overdueActions.length, status: overdueActions.length ? "red" : "green", detail: "Past due and open" },
      { label: "Recent Decisions", value: recentDecisions.length, status: "green", detail: "Latest 5 captured" },
    ] as const;
    const trendCards = [{ label: "This Week", milestones: thisWeekMilestones.length, actions: thisWeekActions.length }, { label: "Next 30 Days", milestones: next30Milestones.length, actions: next30Actions.length }];

    return (
      <div className="grid gap-6" id="dashboard">
        <section className="rounded-3xl border border-slate-200 bg-white p-6"><p className="text-sm font-semibold uppercase tracking-[0.22em] text-blue-700">Executive portfolio dashboard</p><h3 className="mt-2 text-2xl font-bold">Portfolio Health</h3><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Visual KPI cards, RAG indicators, drill-down links, and near-term views for executive review.</p></section>
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{kpis.map((kpi) => <article className={`rounded-3xl border p-5 shadow-sm ${statusTone(kpi.status)}`} key={kpi.label}><p className="text-xs font-bold uppercase tracking-[0.18em]">{kpi.label}</p><p className="mt-3 text-4xl font-black">{kpi.value}</p><p className="mt-2 text-sm font-semibold">{kpi.detail}</p><span className="mt-4 inline-flex rounded-full border bg-white/70 px-3 py-1 text-xs font-black uppercase">{ragLabel(kpi.status)}</span></article>)}</section>
        <section className="rounded-3xl border border-slate-200 bg-white p-6"><h3 className="text-xl font-bold">Portfolio Trends</h3><div className="mt-4 grid gap-4 md:grid-cols-2">{trendCards.map((card) => <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4" key={card.label}><p className="text-sm font-bold uppercase text-slate-500">{card.label}</p><dl className="mt-3 grid grid-cols-2 gap-3"><div><dt className="text-xs font-semibold text-slate-500">Milestones</dt><dd className="text-2xl font-bold">{card.milestones}</dd></div><div><dt className="text-xs font-semibold text-slate-500">Actions</dt><dd className="text-2xl font-bold">{card.actions}</dd></div></dl></div>)}</div></section>
        <section className="rounded-3xl border border-slate-200 bg-white p-6"><h3 className="text-xl font-bold">Projects Requiring Attention</h3>{attentionMilestones.length === 0 ? <p className="mt-4 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600">No milestones currently meet attention rules.</p> : <div className="mt-4 grid gap-3">{attentionMilestones.map(({ milestone, stage, workstream, reasons, daysUntilPlanned }) => <article className="rounded-2xl border border-slate-200 p-4" key={milestone.id}><div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between"><div><button className="text-left font-bold text-blue-700 hover:underline" onClick={() => navigateTo("Workstreams", `workstream-${workstream?.id}`)} type="button">{file.project.name}</button><p className="mt-1 text-sm text-slate-600">Project drill-down · <button className="font-semibold text-blue-700 hover:underline" onClick={() => navigateTo("Workstreams", `workstream-${workstream?.id}`)} type="button">{workstream?.name ?? "Unassigned workstream"}</button> · {stage?.name ?? "Unassigned stage"}</p></div><span className={`rounded-full border px-3 py-1 text-xs font-bold uppercase ${statusTone(milestone.status)}`}>{statusLabel(milestone.status)}</span></div><p className="mt-3 text-sm text-slate-700">{reasons.join("; ")}.</p><button className="mt-2 text-sm font-bold text-blue-700 hover:underline" onClick={() => navigateTo("Workstreams", `workstream-${workstream?.id}`)} type="button">Open milestone / workstream drill-down</button><p className="mt-2 text-xs font-semibold text-slate-500">Planned {milestone.plannedDate} · {formatDateDistance(daysUntilPlanned)} · {formatVariance(milestone)}</p></article>)}</div>}</section>
        <div className="grid gap-6 xl:grid-cols-2">
          <section className="rounded-3xl border border-slate-200 bg-white p-6"><h3 className="text-xl font-bold">Critical Issues</h3>{criticalIssues.length === 0 ? <p className="mt-4 text-sm text-slate-600">No open critical issues.</p> : <div className="mt-4 space-y-3">{criticalIssues.map((issue) => <button className="block w-full rounded-2xl bg-red-50 p-4 text-left hover:ring-2 hover:ring-red-200" key={issue.id} onClick={() => navigateToRaid("Issues", `issue-${issue.id}`)} type="button"><span className={`rounded-full border px-2 py-1 text-xs font-bold uppercase ${statusTone(ragFromIssue(issue))}`}>{ragLabel(ragFromIssue(issue))}</span><p className="mt-2 font-semibold text-red-950">{issue.title}</p><p className="text-sm text-red-800">Owner: {issue.owner || "Unassigned"} · Target {issue.targetResolutionDate}</p></button>)}</div>}</section>
          <section className="rounded-3xl border border-slate-200 bg-white p-6"><h3 className="text-xl font-bold">Top Risks</h3>{topRisks.length === 0 ? <p className="mt-4 text-sm text-slate-600">No active risks.</p> : <div className="mt-4 space-y-3">{topRisks.map((risk) => <button className="block w-full rounded-2xl bg-slate-50 p-4 text-left hover:ring-2 hover:ring-blue-200" key={risk.id} onClick={() => navigateToRaid("Risks", `risk-${risk.id}`)} type="button"><span className={`rounded-full border px-2 py-1 text-xs font-bold uppercase ${statusTone(ragFromRisk(risk))}`}>{ragLabel(ragFromRisk(risk))}</span><p className="mt-2 font-semibold">{risk.title}</p><p className="text-sm text-slate-600">Probability {statusLabel(risk.probability)} · Impact {statusLabel(risk.impact)}</p></button>)}</div>}</section>
          <section className="rounded-3xl border border-slate-200 bg-white p-6"><h3 className="text-xl font-bold">Blocked Dependencies</h3>{blockedDependencies.length === 0 ? <p className="mt-4 text-sm text-slate-600">No blocked dependencies.</p> : <div className="mt-4 space-y-3">{blockedDependencies.map((dependency) => <button className="block w-full rounded-2xl bg-red-50 p-4 text-left hover:ring-2 hover:ring-red-200" key={dependency.id} onClick={() => navigateTo("Dependencies", `dependency-${dependency.id}`)} type="button"><span className={`rounded-full border px-2 py-1 text-xs font-bold uppercase ${statusTone(ragFromDependency(dependency, today))}`}>{ragLabel(ragFromDependency(dependency, today))}</span><p className="mt-2 font-semibold text-red-950">{dependency.title}</p><p className="text-sm text-red-800">Owner: {dependency.owner || "Unassigned"} · Due {dependency.dueDate}</p></button>)}</div>}</section>
          <section className="rounded-3xl border border-slate-200 bg-white p-6"><h3 className="text-xl font-bold">Upcoming Milestones</h3>{next30Milestones.length === 0 ? <p className="mt-4 text-sm text-slate-600">No upcoming incomplete milestones in the next 30 days.</p> : <div className="mt-4 space-y-3">{next30Milestones.slice(0, 8).map(({ milestone, workstream, daysUntilPlanned }) => <button className="block w-full rounded-2xl bg-slate-50 p-4 text-left hover:ring-2 hover:ring-blue-200" key={milestone.id} onClick={() => navigateTo("Workstreams", `workstream-${workstream?.id}`)} type="button"><span className={`rounded-full border px-2 py-1 text-xs font-bold uppercase ${statusTone(milestone.status)}`}>{milestone.status === "not_set" ? "Not set" : statusLabel(milestone.status)}</span><p className="mt-2 font-semibold">{milestone.name}</p><p className="text-sm text-slate-600">{workstream?.name ?? "Unassigned workstream"} · {milestone.plannedDate} · {formatDateDistance(daysUntilPlanned)}</p></button>)}</div>}</section>
          <section className="rounded-3xl border border-slate-200 bg-white p-6"><h3 className="text-xl font-bold">Overdue Actions</h3>{overdueActions.length === 0 ? <p className="mt-4 text-sm text-slate-600">No overdue open actions.</p> : <div className="mt-4 space-y-3">{overdueActions.map(({ action, daysUntilDue }) => <button className="block w-full rounded-2xl bg-red-50 p-4 text-left hover:ring-2 hover:ring-red-200" key={action.id} onClick={() => navigateTo("Governance", `action-${action.id}`)} type="button"><span className={`rounded-full border px-2 py-1 text-xs font-bold uppercase ${statusTone(ragFromAction(action, today))}`}>{ragLabel(ragFromAction(action, today))}</span><p className="mt-2 font-semibold text-red-950">{action.description}</p><p className="text-sm text-red-800">Owner: {action.owner || "Unassigned"} · {formatDateDistance(daysUntilDue)}</p></button>)}</div>}</section>
          <section className="rounded-3xl border border-slate-200 bg-white p-6"><h3 className="text-xl font-bold">Recent Decisions</h3>{recentDecisions.length === 0 ? <p className="mt-4 text-sm text-slate-600">No decisions captured yet.</p> : <div className="mt-4 space-y-3">{recentDecisions.map((decision) => <button className="block w-full rounded-2xl bg-slate-50 p-4 text-left hover:ring-2 hover:ring-blue-200" key={decision.id} onClick={() => navigateTo("Governance", `decision-${decision.id}`)} type="button"><p className="font-semibold">{decision.title}</p><p className="text-sm text-slate-600">{decision.decisionDate} · {decision.decisionMaker || "Decision maker not set"}</p><p className="mt-1 text-sm text-slate-700">{decision.decisionText}</p></button>)}</div>}</section>
        </div>
      </div>
    );
  }
